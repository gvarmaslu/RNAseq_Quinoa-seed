...
# where are we?
# R CMD BATCH 3.2_Features-combine_all.R

basedir <- "/home/gala0002/proj/proj_dir"
setwd(basedir)

#create new dir 
dir.create(paste(basedir, "NG-14833_3.2_Salmon_Quinoa_Genome_combine-all", sep="/"))

#cntdir <- paste(basedir, "3.0_Htseq_trained_PO_PP_PO-PP/PO", sep="/")
#cntdir <- paste(basedir, "NG-14833_4.1_express_pars-tpm", sep="/")
#pat <- "(.*)_express.tpm"

cntdir <- paste(basedir, "NG-14833_3.1_Salmon_Quinoa_Genome_pars-count", sep="/")
pat <- "(.*).count"
...
#data.all.summary <- data[grep("^P", rownames(data), perl=TRUE, invert=TRUE), ]
# review
#data.all.summary
...
#write.csv(data.all.summary, file = "/home/gala0002/proj/proj_Sophie-Laura/P15505_3.2_Salmon_PGSC_v4.03_Genome_combine-all/Express_counts_allsamp_summary.csv")

####################################
# take all data rows to a new table

#data.all <- data[grep("^P", rownames(data), perl=TRUE, invert=FALSE), ]
...
# write data to file
write.table(data, file="/home/gala0002/proj/proj_dir/NG-14833_3.2_Salmon_Quinoa_Genome_combine-all/Express_counts_allsamp.matrix", sep="\t", quote=FALSE, row.names = T) 
